...
void Allocator::loadRooms(const string& file)
{
    ...
    //std::cout<<"the type name is "<<typeid(rooms).name()<<std::endl;
    //std::cout<<"the type name is "<<typeid(new Room[roomCount]).name()<<std::endl;
    totalCapacity = 0;
    int i = 0;
    while (fileio::areMoreRooms()) {

        rooms[i] = fileio::nextRoom();
        //std::cout<<"the type name is "<<typeid(rooms[i]).name()<<std::endl;

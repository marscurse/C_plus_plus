...
class BinaryTree
{
  ...
  private:
     ...
     *  as examples.
     */
          void sumDistances(Node* subRoot,int*  output,int pathlen) const;
          void findpath(Node* head,int pathlen,T path[],vector<vector<T>> &b) const;
     /*helper function for isOrderedRecursive*/
          void isOrderedRecursive(const Node* subRoot,stack<T> &sta) const;
